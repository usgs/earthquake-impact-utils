...
class CopySender(Sender):
    '''Class for sending and deleting files and directories via system copy and delete.

    CopySender creates a remote directory if one does not already exist, and copies files and directories
    ...
    def send(self):
        ...
        This method deletes any previous cancel files that may exist in the remote_directory.

        Returns:
            Tuple containing number of files sent to local directory, and a message describing success.
            
        '''
        #copy directories to remote location, changing remote name to desired alias
        ...
        if self._local_directory:
            ...
            for filename in allfiles:
                self._copy_file_with_path(filename,remote_folder,local_folder=local_folder)
                nfiles += 1

            nfiles += sum([len(files) for r, d, files in os.walk(local_folder)])
        ...
        return (nfiles,'%i files sent successfully using CopySender.' % nfiles)

    def cancel(self,cancel_content=None):
        """Create a cancel file (named as indicated in constructor "cancelfile" parameter) in remote_directory.
    ...
    def _copy_file_with_path(self,local_file,remote_folder,local_folder=None):
