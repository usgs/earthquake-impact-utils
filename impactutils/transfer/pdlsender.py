...
class PDLSender(Sender):
    ...
    Required properties:
      ...
      - configfile String path to PDL config file (specifying remote hubs, ports, etc.)
      - source Source of this product (i.e., contributor of the product). i.e. 'us'
    ...
    def __init__(self,properties=None,local_files=None,local_directory=None,cancelfile='.cancel',product_properties=None):
        """Create a PDLSender object using property settings, local files and directories to transfer/delete.

        :param properties:
          Dictionary of properties that are needed for a PDL
        :param local_files:
        ...
        :param product_properties:
          A dictionary of arbitrary properties that will be passed to PDL as --property-PROPERTY_NAME=PROPERTY_VALUE.
          These properties can be ints, floats, strings, or datetime objects.  Other kinds of objects will be 
          converted to strings using the str() method - unpredictable results may follow.
        
        """
        if product_properties is not None:
            self._product_properties = product_properties.copy()
        
        super().__init__(properties=properties,local_files=local_files,local_directory=local_directory,cancelfile=cancelfile)
    ...
    def send(self):
        ...
        :returns:
          Tuple containing number of files sent, and the standard output of the PDL command.
        """
        ...
        for propkey,propvalue in self._product_properties.items():
            if isinstance(propvalue,float):
                prop_nuggets.append('--property-%s=%.4f' % (propkey,propvalue))
                ...
                prop_nuggets.append('--property-%s=%s' % (propkey,propvalue.strftime(DATE_TIME_FMT)[0:23]))
            elif isinstance(propvalue,str):
                prop_nuggets.append('--property-%s=%s' % (propkey,propvalue))
            else:
                prop_nuggets.append('--property-%s=%s' % (propkey,str(propvalue)))
    ...
    def cancel(self,cancel_content=None):
        ...
        :param cancel_content:
          String containing cancel message.  This is NOT used in the implementation 
          for this class.
        :returns:
