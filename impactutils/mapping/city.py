...
class Cities(object):
    ...
    def __init__(self,dataframe):
        ...
        if len(set(dataframe.columns).intersection(set(self.REQFIELDS))) < 3:
            raise KeyError('Missing some of required keys: %s' % self.REQFIELDS)
    ...
    @classmethod
    def fromDefault(cls):
        """Load the cities data set from the text file included in the distribution for this module.
    ...
    def limitByPopulation(self,pop,minpop=0):
        ...
        if minpop >= pop:
            raise ValueError('Minimum population must be less than population threshold.')
        newdf = self._dataframe.copy()
