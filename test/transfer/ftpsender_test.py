...
def _test_send_file(properties):
    ...
    cancelfile = 'CANCEL'
    try:
        sender = FTPSender(properties=properties, local_files=[thisfile],cancelfile=cancelfile)
        ...
        url = 'ftp://%s%s/%s' % (properties['remote_host'], properties['remote_directory'],cancelfile)
        fh = urllib.request.urlopen(url)
        fh.close()
        ...
        tpl = (thisfile, properties['remote_host'], properties['remote_directory'])
...
def _test_send_folder(properties):
    ...
    cancelfile = 'CANCEL'
    try:
        sender = FTPSender(properties=properties, local_directory=thispath,cancelfile=cancelfile)
        nfiles,send_msg = sender.send()
        url = 'ftp://%s%s' % (properties['remote_host'], properties['remote_directory'])
